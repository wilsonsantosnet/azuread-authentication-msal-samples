...
namespace ClientAD.Controllers.MSAL
{
    ...
    public class HomeController : Controller
    {
        ...
        public HomeController(ILogger<HomeController> logger, IHttpClientFactory clientFactory, IConfiguration config, ITokenAcquisition tokenAcquisition)
        {
            ...
            _scope = new string[] { "api://1ea9629c-d011-40ff-964b-7dfa36c6d9e1/WeatherForecast" };
        }
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                ...
                var respostaB = CallGraphAPI(accessToken2);
                
                //var accessToken3 = await _tokenAcquisition.GetAccessTokenForUserAsync(new string[] { "https://graph.microsoft.com/User.Read.All" });
                var respostaC = Getusers(accessToken2);
            }
            catch (MicrosoftIdentityWebChallengeUserException ex)
            {
                _tokenAcquisition.ReplyForbiddenWithWwwAuthenticateHeader(_scope, ex.MsalUiRequiredException);
                return View();
            }
            catch (MsalUiRequiredException ex)
            {
                _tokenAcquisition.ReplyForbiddenWithWwwAuthenticateHeader(_scope, ex);
                return View();
            }
            catch (Exception ex)
            {
        ...
        private static GraphUser CallGraphAPI(string accessToken2)
        {
            ...
            using (HttpClient userClient = new HttpClient())
            {
                ...
                return System.Text.Json.JsonSerializer.Deserialize<GraphUser>(dataGraphUser);
